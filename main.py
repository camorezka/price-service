...
# Binance блокирует cloud-хостинги (451). Используем зеркало data-api.binance.vision
BINANCE_HOSTS = [
    "https://data-api.binance.vision",  # официальное зеркало без гео-блокировки
    "https://api.binance.com",          # запасной (может дать 451)
]
...
async def fetch_crypto_price(symbol: str, exchange: str):
    ...
    try:
        if exchange == "binance":
            ...
            # Пробуем spot, потом linear (для некоторых монет нет spot-пары)
...
FOREX_NAMES = {
    ...
    "UAH": "Украинская гривна", "JPY": "Японская иена", "CHF": "Швейцарский франк",
    "KZT": "Казахстанский тенге", "GEL": "Грузинский лари", "RUB": "Российский рубль",
}
...
COIN_REVIEWS = {
    ...
    "AVAX": ("Avalanche — быстрый L1 с subnets. Активно развивается, хорош для диверсификации портфеля.", "накапливать"),
...
async def fetch_exchange_coins(exchange: str):
    try:
        if exchange == "binance":
            # Используем зеркало вместо api.binance.com
    ...
    while True:
        try:
            ...
            for row in data:
                ...
                tg_id    = row.get("tg_id")
                # ИСПРАВЛЕНИЕ: берём last_price, при 0 — price_at_add; мониторим даже если цена = 0
                ...
                # Если у нас нет старой цены — просто сохраняем текущую как базовую
                if old_px <= 0:
...
@app.get("/")
async def root():
    return {"status": "ok", "service": "Monitor Space", "version": "4.1"}
...
async def activate_monitor(request: Request):
    try:
        ...
        if existing.data:
            ...
            from datetime import timedelta
            # ИСПРАВЛЕНИЕ: получаем актуальную цену при первом запуске мониторинга
